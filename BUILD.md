...
Bash・rsync・Composer・PHPが必要です。開発用プロジェクトで実行します。
...
開発用のファイルや依存パッケージは変更しません。途中で失敗した出力は配布しないでください。
ZIP作成、アップロード、JavaScriptのビルド、回帰テストは自動実行しません。
